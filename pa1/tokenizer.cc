...
Tokenizer::Tokenizer(std::string ln, std::string parserType) {
    ...
    if(parserType == "formula"){
        for(size_t i = 0; i < ln.length(); i++){
            if ((int(ln[i]) > 32 && int(ln[i]) < 40) || (int(ln[i]) > 45 && int(ln[i])< 48) || (int(ln[i])> 59 &&  int(ln[i]) < 65) || (int(ln[i])> 123) || (int(ln[i])== 58 || (int(ln[i])== 59) ) ){
                throw std::invalid_argument("Error: invalid input");
            }
        }
    }

    else if(parserType == "assignment"){
        for(size_t i = 0; i < ln.length(); i++)
        {
            if ((int(ln[i]) > 32 && int(ln[i]) < 40) || (int(ln[i]) > 45 && int(ln[i])< 48) || (int(ln[i])> 59 &&  int(ln[i]) < 65) || (int(ln[i])> 123)){
                throw std::invalid_argument("Error:invalid input");
            }
        }
    } 
    ...
    for (std::sregex_iterator it = std::sregex_iterator(ln.begin(), ln.end(), reg); it != std::sregex_iterator(); it++) {
        ...
        if (match.str(0).length() > 0 and match.str(0) !=" " ){
            ...
            else if (val.content.compare("0")==0 || val.content.compare("1")==0){
                    val.type = "CONSTANT";
            }
            else{
                val.type = "VARIABLE";
